...
### 1. Vercel 배포: public/ 분리 + api/index.py 구조

**결정**: 정적 파일은 `public/`에, FastAPI는 `api/index.py`에 분리. Vercel이 정적 파일은 CDN으로, API는 Function으로 각각 서빙.  
**이유**: StaticFiles를 FastAPI에 마운트하면 정적 파일도 Serverless Function을 거쳐 cold start 영향과 불필요한 비용이 발생. `public/` 분리 시 CDN 캐싱으로 무료·빠름.  
**로컬 개발**: `uvicorn api.index:app`으로 API만 기동, 정적 파일은 `python -m http.server`나 VS Code Live Server로 별도 서빙. 또는 로컬 전용으로만 StaticFiles 마운트 조건 분기.  
**대안**: StaticFiles 일체형 → 로컬은 편하나 Vercel에서 Function 비용 낭비, CDN 캐싱 불가.
